...
export default function AdminProfilesScreen() {
  ...
  const profilesQuery = useApiQuery<any>(['admin-profiles'], '/api/admin/profiles');
  useEffect(() => {
    if (profilesQuery.data !== undefined) {
      setProfiles(Array.isArray(profilesQuery.data) ? profilesQuery.data : []);
      setLoading(false);
      setRefreshing(false);
    }
  }, [profilesQuery.data]);
  useEffect(() => {
    if (profilesQuery.isError) {
      const status = (profilesQuery.error as any)?.response?.status;
      setError(status === 403 ? 'Admin access required' : 'Failed to load profiles');
      setLoading(false);
      setRefreshing(false);
    }
  }, [profilesQuery.isError]);
  const fetch = () => { void profilesQuery.refetch(); };

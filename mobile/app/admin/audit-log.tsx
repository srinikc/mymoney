...
const ENTITIES = ['', 'expense', 'budget', 'goal', 'investment', 'profile', 'user', 'user.tier', 'feature'];
      ...
      <View style={[styles.card, { backgroundColor: theme.surface }]}>
        ...
        <View style={styles.cardMiddle}>
          ...
          <Text style={[styles.userText, { color: theme.textSecondary }]}>
            {item.profile?.user?.name || item.profile?.user?.email || `User #${item.profile?.userId}`}
          </Text>
        </View>
        {metaDisplay && (
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: theme.surface }]}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[styles.headerTitle, { color: theme.text }]}>Admin — Audit Log</Text>
          <Text style={[styles.headerSub, { color: theme.textSecondary }]}>View all audit log entries</Text>
        </View>
      </View>
      ...
      <View style={[styles.filtersCard, { backgroundColor: theme.surface }]}>
        <View style={styles.filterRow}>
          ...
          {renderPicker(showActionPicker, setShowActionPicker, actionFilter, ACTIONS, (v) => { setActionFilter(v); setPage(1); }, { '': 'All', create: 'Create', update: 'Update', delete: 'Delete', view: 'View', export: 'Export', import: 'Import' })}
          ...
          {renderPicker(showEntityPicker, setShowEntityPicker, entityFilter, ENTITIES, (v) => { setEntityFilter(v); setPage(1); }, { '': 'All', expense: 'Expense', budget: 'Budget', goal: 'Goal', investment: 'Investment', profile: 'Profile', user: 'User', 'user.tier': 'Tier', feature: 'Feature' })}

          <TextInput
            style={[styles.filterInput, { borderColor: theme.border, color: theme.text }]}
            value={userIdFilter}
            onChangeText={(v) => { setUserIdFilter(v); setPage(1); }}
            placeholder="User ID"
        ...
        <View style={styles.filterRow}>
          <TextInput
            ...
            value={fromDate}
            onChangeText={(v) => { setFromDate(v); setPage(1); }}
          ...
          <TextInput
            style={[styles.dateInput, { borderColor: theme.border, color: theme.text }]}
            value={toDate}
            onChangeText={(v) => { setToDate(v); setPage(1); }}
            placeholder="To date"
            placeholderTextColor={theme.textTertiary}
          />
...
const styles = StyleSheet.create({
  ...
  // Filters
  filtersCard: { margin: 16, borderRadius: 14, padding: 14, gap: 10 },
  ...
  filterInput: { borderWidth: 1, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6, fontSize: 12, width: 60, textAlign: 'center' },

...
import { Download, FileText, Search, Sparkles, Lightbulb, AlertTriangle, Loader2, FileBarChart, RefreshCw } from "lucide-react"
...
const KIND_META: Record<string, { label: string; icon: typeof Sparkles }> = {
  ...
  velocity: { label: "Pace", icon: Sparkles },
  subscription: { label: "Subscription", icon: Sparkles },
  "tax-optimization": { label: "Tax", icon: Lightbulb },
  "lifestyle-creep": { label: "Lifestyle", icon: Sparkles },
  seasonal: { label: "Seasonal", icon: Lightbulb },
  "weekend-effect": { label: "Weekend", icon: Sparkles },
}
    ...
    <div className="space-y-6" ref={reportRef}>
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Button variant="outline" size="sm" onClick={downloadFinancialPdf} disabled={generatingPdf}>
            {generatingPdf ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <FileBarChart className="mr-2 h-4 w-4" />}
            {generatingPdf ? "Generating..." : "Full PDF"}
          </Button>
          <Select>
      ...
      <Tabs defaultValue="overview">
        <TabsList>
          ...
          <TabsTrigger value="intelligence">
            ...
            Intelligence
            {(intelCounts.alert > 0 || intelCounts.warn > 0) && (
              <Badge variant="secondary" className="ml-1.5 text-[10px]">
                {intelCounts.alert + intelCounts.warn}
              </Badge>
            )}
          </TabsTrigger>
            ...
            <CardContent>
              {intelligenceLoading ? (
                ...
                <div className="py-12 text-center text-muted-foreground">
                  <Sparkles className="mx-auto h-10 w-10 opacity-40 mb-3" />
                  <p className="text-lg font-medium">No intelligence insights right now</p>
                  <p className="text-sm mt-1">Add more transactions to unlock anomaly, pace, and tax-gap insights.</p>
                </div>
              ) : (
                <>
                  <div className="flex flex-wrap gap-2 mb-4">
                    <Badge variant="outline" className="text-xs">
                      {intelligence.length} insight{intelligence.length === 1 ? "" : "s"}
                    </Badge>
                    {intelCounts.alert > 0 && (
                      <Badge className="text-xs bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/30">
                        {intelCounts.alert} alert{intelCounts.alert === 1 ? "" : "s"}
                      </Badge>
                    )}
                    {intelCounts.warn > 0 && (
                      <Badge className="text-xs bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/30">
                        {intelCounts.warn} warning{intelCounts.warn === 1 ? "" : "s"}
                      </Badge>
                    )}
                    {intelCounts.info > 0 && (
                      <Badge variant="secondary" className="text-xs">
                        {intelCounts.info} info
                      </Badge>
                    )}
                  </div>
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {intelligence.map((item) => {
                      const meta = KIND_META[item.kind] || { label: item.kind, icon: Sparkles }
                      ...
                      return (
                        <div key={item.id} className={`rounded-lg border p-4 ${severityStyle}`}>
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex items-center gap-2">
                              <MetaIcon className={`h-4 w-4 ${item.severity === "alert" ? "text-red-500" : item.severity === "warn" ? "text-amber-500" : "text-primary"}`} />
                              <span className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{meta.label}</span>
                            </div>
                            <Badge className={`text-[10px] ${badgeColor}`}>{item.metric}</Badge>
                          </div>
                          <p className="mt-2 text-sm font-medium">{item.title}</p>
                          <p className="mt-1 text-xs text-muted-foreground">{item.description}</p>
                          {item.actionable && (
                            <p className="mt-2 text-xs flex items-start gap-1.5 text-primary">

  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: theme.surface }]}>
        ...
        <TouchableOpacity onPress={() => { setFormTitle(''); setFormDate(new Date().toISOString().slice(0, 10)); setFormError(null); setShowForm(true); }} style={[styles.addBtn, { backgroundColor: theme.primaryLight }]}><Ionicons name="add" size={22} color={theme.primary} /></TouchableOpacity>
      </View>
      ...
      : <FlatList data={data} keyExtractor={(i, idx) => String(i.id ?? i._id ?? idx)}
            ...
            return (
              <TouchableOpacity style={[styles.card, { backgroundColor: theme.surface }]} onLongPress={() => handleDelete(item.id ?? item._id ?? '')}>
                <View style={styles.cardRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.cardTitle, { color: theme.text }]}>{item.title || item.name || 'Reminder'}</Text>
          ...
          ListEmptyComponent={<View style={styles.empty}><Ionicons name="notifications-outline" size={48} color={theme.textTertiary} /><Text style={{ color: theme.textSecondary, fontSize: 15, fontWeight: '500' }}>No reminders</Text></View>}
        />
      }

      <Modal visible={showForm} transparent animationType="slide">
        <View style={styles.modalOverlay}><View style={[styles.modalCard, { backgroundColor: theme.surface }]}>
          <View style={styles.modalHeader}><Text style={[styles.modalTitle, { color: theme.text }]}>New Reminder</Text><TouchableOpacity onPress={() => setShowForm(false)}><Ionicons name="close" size={24} color={theme.textTertiary} /></TouchableOpacity></View>
